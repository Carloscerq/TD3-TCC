...
class GazeboEnv:
    ...
    def step(self, action):
        ...
        self.pos_x = self.last_odom.pose.pose.position.x
        self.pos_y = self.last_odom.pose.pose.position.y
        ...
        distance = np.linalg.norm(
            [self.pos_x - self.goal_x, self.pos_y - self.goal_y]
        )

        skew_x = self.goal_x - self.pos_x
        skew_y = self.goal_y - self.pos_y
    ...
    def reset_pose(self, orientation):
        pose_msg = PoseWithCovarianceStamped()
        pose_msg.header.frame_id = 'odom'

        pose_msg.pose.pose.position = Point(0,0,0)
    ...
    def reset(self):
        ...
        try:
            self.reset_pose(quaternion)
        except rospy.ServiceException:
            rospy.logerr("SetPose Service call failed: %s", e)

        self.pos_x = object_state.pose.position.x
        self.pos_y = object_state.pose.position.y
    ...
    def get_reward(target, collision, action, min_laser):
        if target:
            ...
            return 50*action[0] - 50*abs(action[1]) - r3(min_laser) 
